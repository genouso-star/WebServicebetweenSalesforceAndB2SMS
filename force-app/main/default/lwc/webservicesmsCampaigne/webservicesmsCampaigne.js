...
export default class webservicesmsCampaigne extends LightningElement {
    logo= logoB2SMS;
    IconArrow= Arrow;
    previousTabValue = '';
    @track dateValue = '';
    ...
    @track username = '';
    @track password = '';
    @track tonameCampagne = '';
    @track isLoginModalOpen = true;
    @track error = false;
    @track errorCampagne = false;
    ...
    connectedCallback() {
        ...
        .then(result => {
            ...
            if (result.isLoggedIn) {
                ...
                pingAPI({ username: result.username, password: result.password })
            .then(result => {
            ...
            .catch(error => {
                this.pingResponse = { error: 'Erreur lors de la récupération des données' };
                console.error(error);
            });
            } else {
                this.isLoading1 = false;
    ...
    //
    handletonomdelaCampagne(event) {
        this.tonameCampagne = event.target.value;
        
        if (this.tonameCampagne) {
            this.nextStepButtonMessage = false;
        } else {
            this.nextStepButtonMessage = true;
        }
    ...
    sendMessage() {  
        if (!this.tonameCampagne) {
            this.errorCampagne = true; // Afficher l'erreur si le champ est vide
            return; // Arrêtez l'exécution de la fonction si le champ est vide
        } else {
            this.errorCampagne = false; // Réinitialiser l'erreur si le champ est rempli
        }
        const toNumbersInput = this.template.querySelector('[data-id="toNumbers"]');
        const messageInput = this.template.querySelector('[data-id="messageInput"]');
    ...
    login() {
        ...
        if (isValid) {
            this.isLoading1 = true;
            validateB2smsCredentials({ username: this.username, password: this.password })
            .then(result => {
                if(result.valid){
                    this.xSmsCredit = result.headers['X-SMS-Credit'];
                    this.isLoading1 = false;
    ...
    handlePredefinedTabClick() {
        // L'utilisateur a cliqué sur l'onglet "Message(s) prédéfini(s)", réinitialiser l'onglet actif à "Message(s) prédéfini(s)"
    ...
    deletePredefinedMessage() {
        if (this.selectedMessageValue) {
            ...
            if (messageToDelete) {
                this.isLoading2 = true;
                deleteMessageById({ messageId: messageToDelete.key })
                    .then(() => {
